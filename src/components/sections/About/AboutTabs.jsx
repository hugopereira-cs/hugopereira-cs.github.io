...
/**
 * Componente para renderizar o conteúdo da aba Habilidades.
 * Exibe badges das tecnologias.
 *
 * @component
 * @param {Object} props
 * @param {string[]} props.techs - Lista de slugs das tecnologias.
 */
 ...
 * @component
 * @param {Object} props
 * @param {Array} props.items - Lista de idiomas e níveis.
 */
  ...
  return (
    <div className="space-y-3 p-4">
      {items.map((language, index) => (
        <div
          key={index}
          className={`flex justify-between items-center ${UI_STYLES.border.bottom} pb-2`}>
          <span className={`${UI_STYLES.text.base} font-medium`}>{language.name}</span>
          <span className={`${UI_STYLES.text.secondary} text-sm`}>{language.level}</span>
        </div>
      ))}
    </div>
  );
 ...
 * @component
 * @param {Object} props
 * @param {Array} props.graduation - Lista de formações acadêmicas.
 * @param {Array} props.courses - Lista de cursos complementares.
 */
          ...
          <div className="space-y-3">
            {graduation.map((edu, index) => (
              <div key={index} className="border-l-2 border-brand-accent-hover pl-4">
                <p className={`${UI_STYLES.text.base} font-medium`}>{edu.course}</p>
                <p className={`${UI_STYLES.text.secondary} text-sm`}>{edu.institution}</p>
                <p className={`${UI_STYLES.text.secondary} text-xs mt-1`}>{edu.period}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {courses.map((course, index) => (
              <div key={index} className="flex justify-between items-start">
                <div>
                  <p className={`${UI_STYLES.text.base} font-medium`}>{course.name}</p>
                  <p className={`${UI_STYLES.text.secondary} text-sm`}>{course.platform}</p>
                </div>
 ...
 * @component
 * @param {Object} props
 * @param {Array} props.items - Lista de experiências profissionais.
 */
      ...
      {items.map((experience, index) => (
        <div key={index} className="border-l-2 border-brand-accent-hover pl-4">
          <div className="flex justify-between items-start mb-2">
            <p className={`${UI_STYLES.text.base} font-medium`}>{experience.role}</p>
            <span className={`${UI_STYLES.text.secondary} text-xs whitespace-nowrap ml-2`}>
              {experience.period}
            </span>
          </div>
          <p className={`${UI_STYLES.text.secondary} text-sm font-medium mb-2`}>
            {experience.company}
          </p>
          <p className={`${UI_STYLES.text.secondary} text-sm`}>{experience.description}</p>
        </div>
      ))}
    </div>
  );
...
export function AboutTabs() {
  ...
  return (
    <section
      className={`w-full max-w-[320px] lg:max-w-4xl lg:mt-8 ${UI_STYLES.border.default} rounded col-span-full flex flex-col mb-4`}>
      <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="flex flex-col w-full">
        {/* Tablist - Headers das abas */}
        <Tabs.List
          className={`flex flex-wrap lg:flex-nowrap ${UI_STYLES.border.bottom}`}
          role="tablist">
          {Object.entries(tabs).map(([key, tab]) => (
            <Tabs.Trigger
              key={key}
              value={key}
              className={`flex-[1_0_50%] lg:flex-1 min-h-12 px-2 py-3 text-sm lg:text-base font-medium
                         ${UI_STYLES.text.secondary} hover:text-brand-text-primary
                         ${UI_STYLES.border.bottom} border-r 
